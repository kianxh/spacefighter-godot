...
func _physics_process(delta: float) -> void:
	if is_pid_active:
		input = controller.update(delta, position.x, target_x)
		apply_force(Vector3(input * power, 0, 0), Vector3.ZERO)

# Update the debug label texts
func _process(delta: float) -> void:
	if(label):
		label.text = ""
		for prop_name in ["input", "target_x", "position", "pid_p_gain", "pid_d_gain"]:
